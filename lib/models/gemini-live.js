...
export class GeminiLive extends BaseModel {
  ...
  _openWebSocket() {
    return new Promise((resolve, reject) => {
      ...
      console.log('[Gemini] Connecting to:', url.replace(/key=.*/, 'key=***'));

      // 15 saniye timeout
      this._connectTimeout = setTimeout(() => {
        const state = this._ws ? this._ws.readyState : 'no ws';
        console.error('[Gemini] Connection timeout (15s). WS readyState:', state);
      ...
      try {
        ...
        console.log('[Gemini] WebSocket created, readyState:', this._ws.readyState);
      } catch (e) {
        console.error('[Gemini] WebSocket creation failed:', e.message);
